...
class _SplashScreenState extends State<SplashScreen>
  ...
  Animation<double> _tw(
    ...
    return Tween<double>(begin: begin, end: end).animate(
      CurvedAnimation(
        parent: _seq,
        curve: Interval(start, endT, curve: curve),
      ),
    );
  }
                  ...
                  return Container(
                    decoration: BoxDecoration(
                      gradient: LinearGradient(
                        begin: Alignment(
                          -0.3 + sin(_bgOff.value * 2 * pi) * 0.15,
                          -1.1,
                        ),
                        end: Alignment(
                          0.7 + cos(_bgOff.value * 2 * pi) * 0.15,
                          1.0,
                        ),
                        colors: const [
                          Color(0xFFFDF8F0),
        ...
        children: [
          Container(
            ...
            decoration: BoxDecoration(
              ...
              shape: BoxShape.circle,
              boxShadow: [
                BoxShadow(color: _orangeDeep.withOpacity(0.35), blurRadius: 6),
              ],
            ),
          ),
  ...
  List<_Particle> _makeParticles() {
    ...
    return List.generate(
      ...
      (i) => _Particle(
        ...
        color: colors[i % 4],
        x: rng.nextDouble(),
        baseY: 0.9 + rng.nextDouble() * 0.3,
        size: 10.0 + rng.nextDouble() * 14,
        speed: 0.06 + rng.nextDouble() * 0.10,
        drift: (rng.nextDouble() - 0.5) * 0.04,
        rot: rng.nextDouble() * pi * 2,
        rotSpd: (rng.nextDouble() - 0.5) * 0.015,
        alpha: 0.05 + rng.nextDouble() * 0.08,
      ),
    );
  }
}
